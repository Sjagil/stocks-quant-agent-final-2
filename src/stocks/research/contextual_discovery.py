...
SOURCE_CLASSIFICATIONS = frozenset(
    {"HIGH_POTENTIAL", "WATCHLIST", "NEUTRAL", "REJECTED"}
)
...
class ContextualDiscoveryPolicy:
    ...
    def load(cls, path: Path) -> "ContextualDiscoveryPolicy":
        ...
        result = cls(
            ...
            tactical_min_absolute_daily_return=float(
                raw["tactical_min_absolute_daily_return"]
            ),
            minimum_research_pool_total_score=float(
                raw["minimum_research_pool_total_score"]
            ),
...
def _validate_timestamp(
    value: Any, *, decision_time: pd.Timestamp, field: str
) -> None:
    if value in (None, ""):
        return
...
def canonicalize_contextual_candidates(
    ...
    if (
        ...
        or int(payload.get("broker_calls", 0)) != 0
        or int(payload.get("order_calls", 0)) != 0
    ):
    ...
    for record in payload.get("records") or []:
        ...
        classification = str(
            record.get("source_classification") or record.get("classification") or ""
        )
        ...
        if not bool(record.get("research_eligible", True)):
            raise ValueError(f"{symbol}: non-research-eligible row leaked into payload")
        research_blockers = [str(item) for item in (record.get("research_blockers") or [])]
        if research_blockers:
            raise ValueError(f"{symbol}: research blockers leaked into candidate pool")
        ...
        research_score = _finite(
            record.get("research_score", total_score),
            field=f"{symbol}.research_score",
        )
        fundamental_score = _finite(
            record.get("fundamental_score"), field=f"{symbol}.fundamental_score"
        )
        technical_score = _finite(
            record.get("technical_score"), field=f"{symbol}.technical_score"
        )
        liquidity_score = _finite(
            record.get("liquidity_score"), field=f"{symbol}.liquidity_score"
        )
        risk_score = _finite(record.get("risk_score"), field=f"{symbol}.risk_score")
        macro_score = _finite(
            record.get("macro_score"), field=f"{symbol}.macro_score", allow_none=True
        )
        market_cap = _finite(
            record.get("market_cap"), field=f"{symbol}.market_cap", allow_none=True
        )
        daily_return = _finite(
            record.get("daily_return"), field=f"{symbol}.daily_return", allow_none=True
        )
        if research_score < policy.minimum_research_pool_total_score:
        ...
        sec_points = _finite(
            sec_overlay.get("sec_overlay_points", 0.0),
            field=f"{symbol}.sec_overlay_points",
        )
        ...
        if sec_context:
            ...
            _validate_timestamp(
                sec_context.get("as_of"),
                decision_time=decision_time,
                field=f"{symbol}.sec_as_of",
            )

        contextual_score = max(0.0, min(100.0, float(research_score + sec_points)))
        asset_type = str(record.get("asset_type") or "UNKNOWN").upper()
        mover_type = str(record.get("mover_type") or "").upper()
        requested_research_lane = str(
            record.get("research_lane") or ""
        ).strip().upper()
        if requested_research_lane not in {
            "CORE",
            "GEM",
            "TACTICAL",
            "WATCHLIST",
        }:
            requested_research_lane = ""
        trade_eligible = bool(record.get("trade_eligible", False))
        shariah_gate = str(record.get("shariah_gate") or "PENDING").upper()
        tactical_candidate = (
            (
                bool(mover_type)
                or (
                    ...
                    >= policy.tactical_min_absolute_daily_return
                )
            )
            and technical_score >= policy.tactical_min_technical_score
        )

        if not trade_eligible:
            lane = (
                "PENDING_SHARIAH"
                if shariah_gate in {"PENDING", "REVERIFY"}
                else "RESEARCH_ONLY"
            )
        elif classification == "HIGH_POTENTIAL":
            lane = (
                "GEM"
                if asset_type == "STOCK"
                and market_cap is not None
                and market_cap < policy.gem_market_cap_ceiling_usd
                else "CORE"
            )
        ...
        if requested_research_lane:
            research_lane = requested_research_lane
        elif classification == "HIGH_POTENTIAL":
            research_lane = (
                "GEM"
                if asset_type == "STOCK"
        ...
        rows.append(
            {
                ...
                "trade_blockers": [
                    str(item) for item in (record.get("trade_blockers") or [])
                ],
                "research_blockers": [],
                "shariah_gate": shariah_gate,
                "shariah_verification_required": bool(
                    record.get("shariah_verification_required", False)
                ),
                "core_candidate": lane == "CORE",
                "gem_candidate": lane == "GEM",
                ...
                **macro,
                ...
                "sec_intelligence_score": float(
                    sec_context.get("sec_intelligence_score", 0.0) or 0.0
                ),
                "sec_overlay_points": sec_points,
                "market_cap": market_cap,
                "median_dollar_volume_20d": record.get(
                    "median_dollar_volume_20d"
                ),
                "bid_ask_spread_bps": record.get("bid_ask_spread_bps"),
                "daily_return": daily_return,
                "mover_type": mover_type or None,
                "shariah_status": record.get("shariah_status"),
                "fundamental_coverage": record.get("fundamental_coverage"),
                "fundamental_applicability": record.get("fundamental_applicability"),
                "selection_reasons": record.get("selection_reasons") or [],
                "warnings": record.get("warnings") or [],
                "decision_time": decision_time,
                "screening_date": screening_date,
                "source": "STOCKS_REFERENCE_CONTEXTUAL_PIT_SCREENER",
                "strategy_assignment": "NOT_YET_GENERALIZED",
    ...
    if not frame.empty:
        ...
        frame = (
            ...
            .drop(columns=["_lane_rank"])
            .head(policy.maximum_candidates)
    ...
    audit = {
        "schema": "canonical_contextual_candidate_discovery_v2",
        "screening_date": str(screening_date),
        "decision_time": decision_time.isoformat(),
        "candidate_count": int(len(frame)),
        "trade_eligible_count": int(frame["trade_eligible"].sum()) if not frame.empty else 0,
        "research_only_count": int((~frame["trade_eligible"]).sum()) if not frame.empty else 0,
        "core_count": count_equals("lane", "CORE"),
        "gem_count": count_equals("lane", "GEM"),
        "tactical_count": count_equals("lane", "TACTICAL"),
        "watchlist_count": count_equals("lane", "WATCHLIST"),
        "pending_shariah_count": count_equals("lane", "PENDING_SHARIAH"),
        "pure_research_count": count_equals("lane", "RESEARCH_ONLY"),
        "research_core_count": count_equals("research_lane", "CORE"),
        "research_gem_count": count_equals("research_lane", "GEM"),
        "research_tactical_count": count_equals("research_lane", "TACTICAL"),
        "research_watchlist_count": count_equals("research_lane", "WATCHLIST"),
        "neutral_source_count": count_equals("source_classification", "NEUTRAL"),
        "rejected_source_recovered_count": count_equals(
            "source_classification", "REJECTED"
        ),
        "sec_covered_count": int((frame["sec_event_count"] > 0).sum())
        if not frame.empty
        else 0,
        "sec_positive_overlay_count": int((frame["sec_overlay_points"] > 0).sum())
        if not frame.empty
        else 0,
        "sec_negative_overlay_count": int((frame["sec_overlay_points"] < 0).sum())
        if not frame.empty
        else 0,
        "macro_go_count": count_equals("macro_data_status", "GO"),
        "ranking_score": "RENORMALIZED_RESEARCH_SCORE_PLUS_BOUNDED_SEC_OVERLAY",
        "trade_and_research_eligibility_separated": True,
        "sec_standalone_entry_allowed": False,
        "validated_strategy_assignment": False,
        "reason": (
            "dynamic-universe strategy generalization must run before strategy assignment"
        ),
        "execution_authority": "NONE",
        "broker_calls": 0,
        "order_calls": 0,
