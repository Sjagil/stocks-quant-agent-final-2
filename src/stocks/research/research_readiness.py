...
def build_research_readiness(
    ...
    matrix = _csv(
        root / "artifacts/research_runtime/candidate_strategy_matrix/matrix.csv"
    )
    opportunities = _csv(
        root / "artifacts/research_runtime/candidate_strategy_matrix/opportunities.csv"
    )
    holdout = _json(
        root / "artifacts/research_runtime/unseen_1h_hydration/audit.json"
    )

    generalized = int(
        (
            roster.get("roster_status", pd.Series(dtype=str))
    ...
    shariah_tradeable = int(
        contextual.get("trade_eligible", pd.Series(dtype=bool))
        .fillna(False)
        .astype(bool)
        .sum()
    ...
    if shariah_tradeable == 0:
        live_blockers.append("NO_SHARIAH_VERIFIED_TRADEABLE_CANDIDATES")
    if pending_15m > 0:
        live_blockers.append(
            "15M_BROAD_GENERALIZATION_PENDING_FOR_MARKET_STRUCTURE_FAMILY"
        )
    ...
    return {
        ...
        "shariah_verified_tradeable_candidates": shariah_tradeable,
...
def write_research_readiness(
    ...
    output = root / "artifacts/research_runtime/research_readiness"
