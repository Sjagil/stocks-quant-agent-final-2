...
def _refresh_decisions(root: Path, *, refresh_screener: bool = False) -> list[dict[str, Any]]:
    ...
    steps = []
    if refresh_screener:
        steps.append((["scripts/run_production_screener_v2_44.py"], "PRODUCTION_SCREENER"))
    steps.extend([
    ...
    *,
    ...
    try:
        ...
        if _due(store, "last_decision_refresh_at", int(cfg["runtime"]["decision_refresh_seconds"]), force=force_decision_refresh):
            refresh_screener = _due(
                store,
                "last_screener_refresh_at",
                int(cfg["runtime"].get("screener_refresh_seconds", 21600)),
                force=force_decision_refresh,
            )
            components = _refresh_decisions(root, refresh_screener=refresh_screener)
            summary["components"].extend(components)
            screener_component = next(
                (item for item in components if item.get("component") == "PRODUCTION_SCREENER"),
                None,
            )
            if screener_component and screener_component.get("returncode") == 0:
                store.set("last_screener_refresh_at", _now_iso())
